...
import { buscarProduto, atualizarProduto } from '../../api/produtos'
...
export function EditarProdutoPage() {
  ...
  useEffect(() => {
    if (!id) return
    Promise.all([buscarProduto(Number(id)), listarCategorias()])
      .then(([prodRes, catRes]) => {
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl shadow-sm p-8 space-y-5">
          <div>
            ...
            <input {...register('nome', { required: 'Obrigatório' })} className={inputClass} />
          ...
          <div>
            ...
            <input
              ...
              step="0.01"
              min="0"
              {...register('preco', { required: 'Obrigatório', min: { value: 0.01, message: 'Deve ser maior que zero' } })}
